...
### 机洗模式判断条件
- 前三步（测漏、清洗、漂洗）的 `sequence_no` 分别为 `1`、`2`、`3`
- 如果漂洗（`sequence_no: '3'`）的下一步 `sequence_no` 为 `4`，则为**普通清洗模式**
- 如果漂洗（`sequence_no: '3'`）的下一步 `sequence_no` 为 `7`，则为**机洗模式**
...
2. 清洗 (sequence_no: '2')
3. 漂洗 (sequence_no: '3')
4. 机洗 (sequence_no: '7') - **合并了消毒、终末漂洗、干燥三个步骤**

## 功能实现

### 1. 步骤显示
- **初始状态**：进度条显示所有6个步骤（测漏、清洗、漂洗、消毒、终末漂洗、干燥）
- **机洗模式**：当检测到 `sequence_no: '7'` 的步骤时，自动将后三步（消毒、终末漂洗、干燥）合并显示为一步"机洗"
...
4. 系统会自动根据 `sequence_no` 判断是否为机洗模式，无需额外配置
